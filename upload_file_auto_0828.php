...
function get_burn_down_stats($week, $fdt, $rel){
  global $mysql_con;
  log_update("get_burn_down_stats for: " . $week . $fdt . $rel);

  $result = mysqli_query($mysql_con, "SELECT * FROM BurnDownStats " . "WHERE week = " . "'". "$week" . "'" . " AND fdt = "  . "'" . "$fdt" . "'"  . " AND rel = "  . "'" . "$rel" . "'");
  ...
  mysqli_free_result($result);
  mysqli_query($mysql_con, "INSERT INTO BurnDownStats (week, fdt, rel, incoming, handled, open) VALUES('$week', '$fdt','$rel',0, 0, 0)"); 
  

  return $default_stats;
}

function update_burn_down_stats($week, $fdt, $rel, $stats){
  global $mysql_con;
  ...
  log_update("UPDATE BurnDownStats SET incoming = '$incoming',handled = '$handled',open = '$open' WHERE week = '$week' AND fdt = '$fdt' AND rel = '$rel'");
  mysqli_query($mysql_con, "UPDATE BurnDownStats SET incoming = '$incoming',handled = '$handled',open = '$open' WHERE week = '$week' AND fdt = '$fdt' AND rel = '$rel'");
...
function is_skipped($state, $engineer, $sev, $row, $fdt, $plan_rel, $ec){
    log_update("is_skipped: " . $row["state"] . "=" . $state  . "? " . $row["engineer"] . "=" . $engineer . "? " . $row["severity"] . "=" . $sev . "? " . $row["fdt"] . "=" . $fdt . "? " . $row["plan_rel"] . "=" . $plan_rel . "?");
